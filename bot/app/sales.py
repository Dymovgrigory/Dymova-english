...
- Когда факт есть в контексте — отвечай уверенно и конкретно, без «наверное», \
«возможно» и «кажется».
- Если точного ответа в контексте НЕТ, начни ответ ровно с токена [UNKNOWN], \
а затем одной короткой фразой мягко скажи, что этой информацией не обладаешь и \
точнее подскажет администратор. Не добавляй выдуманных деталей.
