...
def _homework_user_prompt(note: str) -> str:
    ...
    return (
        ...
        "самостоятельно.\n"
        "Если задание по английскому языку — пример и его решение напиши "
        "НА АНГЛИЙСКОМ, объяснение шагов по-русски. Пример строго ОДИН: "
        "одно предложение / один пункт, даже если в задании их несколько."
        f"{extra}"
        f"{_FORMAT_TEMPLATE}"
    )
...
def _homework_text_user_prompt(task_text: str) -> str:
    return (
        ...
        "реши его по шагам, потом дай план, как выполнить своё задание "
        "самостоятельно.\n"
        "Если задание по английскому языку (грамматика, лексика, письмо, "
        "перевод, английские слова) — пример и его решение напиши НА "
        "АНГЛИЙСКОМ, а объяснение шагов по-русски.\n"
        "Пример строго ОДИН: одно предложение, один город, одно слово — "
        "даже если в задании несколько пунктов. Не разбирай все пункты."
        f"{_FORMAT_TEMPLATE}"
    )
...
_FORMAT_TEMPLATE = (
    ...
    "Образец блока «✏️ Похожий пример» для задания по английскому "
    "(задание: напиши о погоде в 4 городах — Москва, Рим, Париж, Каир):\n"
    "✏️ Похожий пример\n"
    "Возьмём город, которого нет в задании, — Лондон.\n"
    "1) It will be rainy in London tomorrow.\n"
    "2) Мы использовали конструкцию It will be + погода, потому что говорим "
    "о прогнозе на завтра.\n"
    "Обрати внимание: пример один (один город), и его нет среди пунктов "
    "задания."
)
