...
def _homework_system_prompt() -> str:
    return (
        ...
        "На фото может быть НЕСКОЛЬКО заданий или упражнений. Внимательно найди ВСЕ задания "
        "на изображении и разбери КАЖДОЕ по очереди, ничего не пропуская. Нумеруй задания так "
        "же, как на фото (Задание 1, Задание 2 и т. д.). "
        "По каждому заданию дай: 1) что оно просит, 2) как его выполнить, 3) правильный пример "
        "или ответ. В самом конце добавь один короткий поддерживающий совет для родителя и ребёнка. "
        "Пиши простыми словами, без длинных вступлений и без markdown-разметки. "
        "Это подсказка, поэтому обязательно советуй проверить ответ с учителем. "
...
def _homework_user_prompt(note: str) -> str:
    ...
    lines.extend(
        [
            ...
            "По каждому заданию простыми словами объясни, что требуется, как это сделать, "
            "и дай правильный пример или ответ.",
