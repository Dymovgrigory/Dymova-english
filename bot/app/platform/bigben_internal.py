...
async def create_income(*, type_id: int, summ: int, bycard: int,
    ...
    Дату поступления пульт проставляет сам (момент создания) — поле date
    в запросе игнорируется, поэтому доход имеет смысл писать сразу после
    подтверждения оплаты, а не задним числом.
    """
