...
class Settings(BaseSettings):
    ...
    # Способ оплаты (справочник /user/incomes/payment-methods): 2 — «Онлайн».
    # Также: 0 — наличными, 1 — терминал, 4 — из приложения, 5 — на сайте,
    # 7 — на расчётный счёт. Платежи мобильного приложения школы идут
    # с кодом 4, наши сайтовые — «Онлайн», чтобы контуры различались.
    CRM_INCOME_BYCARD: int = 2
    ...
    # разделы 2/6). ВЫКЛЮЧЕНА: гейт «поделитесь номером» обрывал диалоги —
    # люди уходили, не дойдя до ответа, а кнопка request_contact есть не во
    # всех клиентах/платформах. Номер по-прежнему подхватывается из текста
    # (_capture_entities), а присланный контакт обрабатывается identify.handle_contact.
