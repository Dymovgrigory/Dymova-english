...
class Settings(BaseSettings):
    ...
    # Создавать ли ещё и счёт ученика (POST /user/payments). НЕТ, и включать
    # не следует — проверено на живой CRM 2026-09-10:
    # 1. API пульта не умеет помечать счёт оплаченным (полей summ_paid/
    #    paydate нет ни в создании, ни в обновлении, маршрута «провести
    #    оплату» тоже нет) — счёт вечно висит в карточке как долг ученика;
    # 2. проведение оплаты по счёту руками в пульте СОЗДАЁТ доход, то есть
    #    поверх нашего получилось бы задвоение денег в кассе;
    # 3. отмена счёта КАСКАДНО отменяет привязанный доход
    #    (cancel_reason: «Удаление счёта») — деньги молча исчезают из кассы.
    # Поэтому пишем только доход, без user_payment_id: он ни от чего не
    # зависит и отменить его может лишь явное действие в пульте.
    CRM_CREATE_INVOICE: bool = False
    ...
    TRIAL_PRICE_120_RUB: int = 2250  # сдвоенные занятия 90+ минут
    TRIAL_PRICE_60_RUB: int = 1125   # занятия 55-89 минут
    ...
    # Таблица соответствия «группа → педагог» (JSON-объект {"group_id": "ФИО"}).
    # В API BigBen педагогов нет; владелец заполняет вручную, имеет
