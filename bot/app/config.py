...
class Settings(BaseSettings):
    ...
    # Создавать ли ещё и счёт ученика (POST /user/payments). По умолчанию нет:
    # API пульта не умеет помечать счёт оплаченным (полей summ_paid/paydate
    # нет ни в создании, ни в обновлении, отдельного «провести оплату» тоже
    # нет), поэтому созданный нами счёт вечно висит в карточке неоплаченным
    # и выглядит как долг ученика. Доход в кассе при этом виден корректно.
    # Включать, только если появится способ провести оплату по счёту.
