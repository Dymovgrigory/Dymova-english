      ...
      {units.map((unit, ui) => (
        <li key={unit.id} className="relative">
          {ui < units.length - 1 ? (
            <span
              aria-hidden
              className="absolute left-[1.65rem] top-16 bottom-[-2.5rem] w-1 rounded-full bg-gradient-to-b from-[#3a2953]/25 to-[#f5ed75]/40"
            />
          ) : null}
          <div
            className={`relative overflow-hidden rounded-[28px] border-2 p-5 ${
              unit.locked ? "border-[#241a30]/10 bg-white/70 opacity-60" : "border-[#3a2953]/15 bg-white shadow-[0_12px_0_rgba(58,41,83,0.06)]"
            }`}
          >
            <div
            ...
            />
            <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#3a2953]/45">
              {unit.book_ru || "Модуль"} · {ui + 1}/{units.length}
            </p>
            <h2 className="mt-1 font-[family-name:var(--font-display)] text-2xl font-extrabold text-[#3a2953]">
              {unit.title_ru}
            </h2>
            <p className="mt-1 text-sm text-[#241a30]/60">{unit.topic_ru}</p>
            <div className="mt-5 flex flex-wrap justify-center gap-x-6 gap-y-5">
              {unit.lessons.map((lesson, i) => {
                      ...
                      className={`grid place-items-center border-4 text-lg font-black transition ${
                        ...
                        now
                          ...
                          : done
                            ...
                            : locked
                              ? "border-[#e8e0d0] bg-[#efe8d8] text-[#241a30]/30"
                              : exam
                                ? "border-[#ee7349] bg-[#3a2953] text-[#f5ed75]"
                                : "border-[#3a2953]/20 bg-[#f5ed75] text-[#241a30]"
                      }`}
                    >
                      {locked ? "•" : done ? "★" : exam ? "🏅" : i + 1}
                    </span>
                    <span className="max-w-[5.5rem] text-center text-[11px] font-extrabold leading-tight text-[#3a2953]">
                      {lesson.title_ru}
                    </span>
                ...
                return (
                  <Link key={lesson.id} href={`/learn/${lesson.id}`} className="hover:brightness-105">
                    {inner}
                  </Link>
                );
              })}
