...
def preparation_sections():
    ...
    pc = price_card("", "7 000 ₽<span class=\"fxb-now-per\">/мес</span>", "2 раза в неделю по 60 минут", [
...
def zayavka_modal():
    """Форма заявки. Шесть полей одним столбцом не влезали в экран (929px при
    окне 900px) и разъезжались по ширине: у input браузерный дефолт content-box,
    у select — border-box. Поэтому: явная блочная модель, две смысловые группы,
    парные поля в строку, прокрутка внутри при закреплённых шапке и кнопке."""
    return ('<div class="fxb-modal fxb-zmodal" id="fxb-zayavka-modal" hidden>'
            ...
            '<p class="fxb-zsub">Перезвоним, расскажем расписание и подберём удобный формат.</p>'
            '</div>'
            ...
            '<div class="fxb-zscroll">'
            '<div class="fxb-zgroup">'
            '<p class="fxb-zgroup-l">О ребёнке</p>'
            '<div class="fxb-zrow">'
            '<label class="fxb-zfield"><span>Имя ребёнка</span><input type="text" name="ChildName" required aria-required="true" placeholder="Как зовут"></label>'
            '<label class="fxb-zfield"><span>Дата рождения</span><input type="date" name="BirthDate" required aria-required="true" min="2007-01-01" max="2025-12-31"></label>'
            '</div>'
            ...
            '</div>'
            '<div class="fxb-zgroup">'
            '<p class="fxb-zgroup-l">Как с вами связаться</p>'
            '<div class="fxb-zrow">'
            ...
            '</div>'
            '</div>'
            '<div class="fxb-zalt"><span class="fxb-zalt-l">Удобнее написать или позвонить?</span>%s</div>'
            '</div>'
            '<div class="fxb-zfoot">'
            '<div class="js-errorbox-all" style="display:none"><div class="t-form__errorbox-text js-rule-error js-rule-error-all" style="color:#e0526a;font-size:13px;font-weight:600;margin-bottom:8px"></div></div>'
            ...
            '<h3 class="fxb-ztitle">Заявка отправлена</h3>'
            '<p class="fxb-zsub">Спасибо! Мы свяжемся с вами. Можно также написать нам:</p>'
            '%s'
            '</div></div></div>'
            % (_zalts(), SEND_ICON, OK_ICON, _zalts()))
...
#fxb-page .fxb-modal[hidden]{display:none}
#fxb-page .fxb-modal__ov{position:absolute;inset:0;background:rgba(20,12,36,.72);backdrop-filter:blur(4px)}
#fxb-page .fxb-modal__box{position:relative;z-index:1;width:min(900px,100%);background:#fff;border-radius:22px;box-shadow:0 40px 90px -30px rgba(0,0,0,.6);animation:fxbpop .3s ease}
#fxb-page .fxb-modal__x{position:absolute;top:14px;right:14px;z-index:2;width:38px;height:38px;border-radius:50%;border:0;cursor:pointer;background:rgba(57,40,82,.06);color:var(--ink);display:grid;place-items:center;transition:background .2s}
#fxb-page .fxb-modal__x:hover{background:rgba(57,40,82,.12)}
#fxb-page .fxb-modal__x svg{width:20px;height:20px;stroke:currentColor;stroke-width:2.2}
@keyframes fxbpop{from{opacity:0;transform:translateY(18px) scale(.96)}to{opacity:1;transform:none}}
#fxb-page .fxb-zbox{width:min(560px,100%);max-height:min(88vh,780px);display:flex;flex-direction:column;overflow:hidden;text-align:left}
#fxb-page .fxb-zview--form{display:flex;flex-direction:column;min-height:0;flex:1}
#fxb-page .fxb-zhead{padding:30px 32px 18px;flex:0 0 auto}
#fxb-page .fxb-zform{display:flex;flex-direction:column;min-height:0;flex:1}
#fxb-page .fxb-zscroll{flex:1 1 auto;min-height:0;overflow-y:auto;padding:0 32px 8px;display:grid;gap:22px;align-content:start;-webkit-mask:linear-gradient(180deg,#000 calc(100% - 26px),transparent 100%);mask:linear-gradient(180deg,#000 calc(100% - 26px),transparent 100%)}
#fxb-page .fxb-zfoot{flex:0 0 auto;padding:16px 32px 24px;border-top:1px solid rgba(57,40,82,.09);background:#fff}
#fxb-page .fxb-zgroup{display:grid;gap:14px}
#fxb-page .fxb-zgroup-l{font-size:15px;font-weight:800;color:var(--ink);margin:0}
#fxb-page .fxb-zrow{display:grid;grid-template-columns:1fr 1fr;gap:12px}
#fxb-page .fxb-ztitle{font-size:23px;font-weight:900;letter-spacing:-.015em;margin-bottom:6px;color:var(--ink);padding-right:44px}
#fxb-page .fxb-zsub{font-size:14px;font-weight:500;color:var(--muted);line-height:1.55}
#fxb-page .fxb-zfield{display:grid;gap:6px}
#fxb-page .fxb-zfield span{font-size:13px;font-weight:600;color:var(--muted)}
#fxb-page .fxb-zfield input,#fxb-page .fxb-zfield select{box-sizing:border-box;width:100%;min-width:0;font-family:inherit;font-size:15px;font-weight:500;color:var(--ink);padding:13px 15px;border:1.5px solid rgba(57,40,82,.13);border-radius:12px;background:#faf8fe;transition:border-color .18s,box-shadow .18s,background .18s}
#fxb-page .fxb-zfield input:hover,#fxb-page .fxb-zfield select:hover{background:#f6f2fd}
#fxb-page .fxb-zfield input::-webkit-calendar-picker-indicator{opacity:.5;cursor:pointer}
...
#fxb-page .fxb-zsubmit{width:100%;justify-content:center;border:0;cursor:pointer;font-family:inherit}
#fxb-page .fxb-zsubmit svg{width:18px;height:18px;stroke:currentColor}
#fxb-page .fxb-znote{font-size:11.5px;color:#a89fbd;font-weight:500;line-height:1.45;margin-top:10px;text-align:center}
#fxb-page .fxb-znote a{color:var(--purple-2);text-decoration:underline}
#fxb-page .fxb-zalt{padding-top:18px;border-top:1px solid rgba(57,40,82,.09)}
#fxb-page .fxb-zalt-l{display:block;text-align:center;font-size:13px;font-weight:500;color:var(--muted);margin-bottom:12px}
...
#fxb-page .fxb-zview--thanks{text-align:center;padding:38px 32px 34px}
@media(max-width:560px){
  #fxb-page .fxb-zrow{grid-template-columns:1fr}
  #fxb-page .fxb-zhead{padding:26px 22px 14px}
  #fxb-page .fxb-zscroll{padding:0 22px 8px}
  #fxb-page .fxb-zfoot{padding:14px 22px 20px}
  #fxb-page .fxb-zbox{max-height:92vh}
}
